...
export const projects: Project[] = [
  ...
  },
  {
    ...
    tools: ['Machine Learning', 'Gaming Analytics', 'pandas', 'scikit-learn', 'matplotlib', 'seaborn'],
    categories: ['Data & ML'],
  ...
  },
  {
    ...
    description:
      'Spotify Wrapped only happens once a year. I wanted my listening data all year round. A fully static, responsive web app with top tracks, artists, a deep analysis tab — genre and popularity breakdowns, quadrant scatter charts, and more — plus a global search across your library and Spotify\'s own catalogue, all installable on your phone, no backend required.',
    url: 'https://github.com/Lanthanum89/spotify-stats-app',
    languages: ['JavaScript', 'CSS', 'HTML'],
    tools: ['Spotify API', 'PWA', 'OAuth PKCE', 'SVG'],
    categories: ['Tools & Apps'],
    pushedAt: '2026-07-26T18:51:29Z',
    featured: true,
    story: {
      ...
      attempts: [
        ...
        'Replaced the fixed-count Up Next/Recently Played lists with a fill-to-fit layout that measures the real DOM and keeps adding rows until the side column matches the hero column\'s height, so it uses exactly as much space as actually fits on any screen instead of leaving gaps or overflowing.',
        'Added a dedicated Search tab that matches instantly against whatever library data (top tracks, artists, recently played) is already loaded, plus live results from Spotify\'s own catalogue — tracks, artists, albums, playlists — with play buttons that start real playback via Spotify Connect.',
        'Put a persistent search bar in the top header so search didn\'t need its own tab visit, then turned it into a live quick-results dropdown — library matches instantly, Spotify results after a short debounce — with the rest of the app dimmed behind it like a modal.',
        'Tried an ambient backdrop behind the Overview now-playing hero — the current track\'s cover art blurred and crossfaded behind the hero row — then reverted it.',
      ],
      challenge: 'Working within Spotify\'s API and auth flow without a server to hold a Client Secret, once I decided to actually host it instead of keeping it local-only. Later, being honest in the UI about which stats update with the range selector (genuinely computed by Spotify) versus which are always just the last 50 streams (Spotify\'s hard cap on play history) — the two were easy to conflate at a glance. More recently, keeping the header\'s quick-results dropdown and the full Search tab\'s own search state independent, so typing in one didn\'t stomp on the other, while still sharing the same result-rendering logic between them.',
      tradeoffs: 'Chose to migrate from a server-held Client Secret to browser-side PKCE auth, trading a small Express backend for something deployable anywhere static files are served, GitHub Pages included. Also reverted the ambient artwork backdrop behind the now-playing hero once it shipped — the extra visual texture wasn\'t worth trading against the app\'s plain, readable stats-first look.',
      next: 'Track changes in top tracks and artists over time, instead of only ever showing a live snapshot.',
      hindsight: 'I did eventually split the OAuth handling into its own module — should have done that from the very first version instead of untangling it from the rest of the app later. The header search also went through three shapes (plain box, jump-to-tab, live dropdown) before landing — I\'d have prototyped the dropdown behaviour first instead of shipping the simpler jump-to-tab version on the way there.',
    },
  },
  {
    ...
    story: {
      ...
      attempts: [
        ...
        'Added Weather detail (feels-like temperature, chance of rain, a five-hour forecast, and a next-day summary), Guardian headlines, and live Reading Buses departures for configured home/work stops — each pulled from a different kind of API (a plain key in the query string, an unauthenticated search endpoint, a raw SIRI-SM XML feed) rather than one uniform integration pattern.',
        'Reworked Email to link out to Gmail instead of rendering message bodies in-app, and added independent unread/starred filter pills plus a hover state matching Calendar\'s event rows.',
        'Added a live rain radar (RainViewer tiles via Leaflet), which moved between the Weather and Guardian cards a few times before landing as its own themed panel, plus fixes for a marker-icon path bug and radar tiles not rendering in portrait.',
        'Added a Sun & Moon widget (sunrise, sunset, moon phase, UV risk), then folded it back into Weather and replaced its old slot with a Countdown widget for a user-set target date.',
        'Added a GitHub activity widget, grown from a one-line strip into a multi-line recent-activity feed, and settled on hardcoding the GitHub username after first trying an env var, then a repo variable.',
        'Switched the type pairing from Playfair Display/Inter/JetBrains Mono to Sora + IBM Plex Mono, and added a manual refresh button next to the theme toggle.',
        'Went through a long run of height and layout fixes chasing a scroll-free landscape-tablet layout as widgets kept being added, removed, and resized — matching Clock\'s height to Calendar/Buses, repeatedly capping and re-capping row heights, and stopping pull-to-refresh from stretching the whole dashboard past 100dvh on mobile.',
      ],
      challenge: 'Getting Google and Spotify auth to both survive page reloads without a backend to hold a session. Google\'s implicit flow has no refresh token, so tokens are cached in localStorage and the widgets restore silently if the cached token\'s still valid, instead of forcing a reconnect on every navigation. Later, discovering the Reading Buses API sends no CORS header at all — confirmed in the browser console, not guessed at — meaning live departure times can\'t be fetched client-side no matter how the request is built. Worked around it two ways: a small Cloudflare Worker that fetches server-side and holds the real API token, and, since the Worker needs manual deployment, a fallback that shows configured stops with pre-set labels and links out to the operator\'s own live-departures page instead, so the feature works even before the Worker exists. More recently, RainViewer\'s tiles only render natively up to zoom level 7, so the radar has to clamp its zoom rather than let a tablet\'s pinch-to-zoom request tiles that don\'t exist — and keeping the whole page scroll-free in landscape got harder with every widget added, since almost every new one needed another round of height caps on its neighbours.',
      tradeoffs: 'Chose to drop mode-switching entirely rather than keep patching it — one well-designed responsive layout turned out simpler to reason about than a set of layouts that all had to stay in sync with each other. Also hardcoded the GitHub username instead of keeping it configurable through an env var or repo variable — it\'s a single-user dashboard, so a constant was simpler than a setting that would only ever be set once.',
      next: 'Tap-to-cycle widget sizes, or a proper drag-and-resize layout, instead of a fixed grid.',
      hindsight: 'I\'d have picked the final dark-lilac look and the single-page layout from the start — the modes concept and the mid-century redesign were both fully working detours before landing on what actually felt right. With this many widgets now sharing a handful of grid rows, I\'d also build the layout to size itself from actual content earlier, rather than hand-tuning fixed height caps after every widget added or removed.',
    },
  },
  {
    name: 'Squish Pop',
  ...
  },
  {
    ...
    tools: ['PWA', 'Fitness Tracker', 'localStorage', 'SVG'],
    categories: ['Tools & Apps'],
  ...
  },
  {
    ...
    tools: ['tkinter', 'PWA', 'London Underground', 'Vite', 'BFS Pathfinding'],
    categories: ['Tools & Apps'],
  ...
  },
  {
    ...
    description:
      'It got annoying flipping between tabs to see where Claude was at. A tiny always-on-top WinForms panel, wired to Claude Code\'s hooks, that recolours like a traffic light: grey idle, amber running, red waiting on you, green done, one row per session.',
    ...
    story: {
      ...
      attempts: [
        ...
        'Reworked the hook script to read the session id and project folder Claude Code already pipes to every hook, writing one status file per session instead of a single global one.',
        'Redrew the overlay as a small panel instead of a lone dot: one row per active session, each a coloured dot plus the project folder name as a label, with a row disappearing once its session ends.',
      ],
      challenge: 'Keeping the panel from feeling cluttered as sessions come and go: resizing and repositioning it live as rows are added or removed, and telling apart two sessions open in the same project folder.',
      tradeoffs: 'Chose to label rows by project folder name rather than the raw session id, trading a small chance of ambiguous labels (disambiguated with a "(1)", "(2)" suffix) for something actually readable at a glance.',
      next: 'Click a row to focus that session\'s terminal window, instead of just seeing its status.',
  ...
  },
  {
    ...
    slug: 'thinkstation',
    description:
      'Wanted to recreate the electronic Countdown handheld I had as a kid. A retro-styled puzzle PWA with Letters, Numbers, and Conundrum rounds, right down to the LCD screen and LED countdown strip.',
    url: 'https://github.com/Lanthanum89/ThinkStation',
    languages: ['TypeScript', 'CSS', 'HTML'],
    tools: ['PWA', 'Game Design'],
    categories: ['Games'],
    pushedAt: '2026-07-24T10:15:35Z',
    story: {
      problem: 'Wanted to recreate a Lexibook electronic Countdown handheld I had as a kid, not just build a generic word game.',
      attempts: [
        'Modelled the physical device first: an LCD screen, an LED strip that burns down over 30 seconds, and chunky plastic buttons, before writing any game logic.',
        'Built the Letters round against a bundled ~150k word dictionary, so entries are validated for real and there\'s a best-word reveal, not just an honesty system.',
        'Wrote a proper Numbers round solver, brute-forcing every combination of the six drawn tiles with +, -, x, / to find an exact or closest match to the target, the same problem the show\'s "Countdown" computer solves live.',
        'Added a Conundrum round and a Full Game mode that chains rounds together with a running score, mirroring the handheld\'s practice vs. TV Challenge modes.',
      ],
      challenge: 'Getting the Numbers solver right: it has to consider every subset and order of the six numbers, not just the numbers used all together, since real solutions often only use four or five of them.',
      tradeoffs: 'Chose to bundle a real dictionary and write an actual solver rather than fake the "expert" reveals, trading a larger bundle size for answers that are always genuinely valid.',
      next: 'A two-player pass-and-play mode, closer to how the physical handheld was actually used head-to-head.',
